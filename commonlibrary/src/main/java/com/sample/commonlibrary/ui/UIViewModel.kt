...
package com.sample.commonlibrary.ui
...
import com.sample.commonlibrary.utils.*
...
class UIViewModel(val activity: Application) : AndroidViewModel(activity) {
    ...
    val charactersItemTextColor: ObservableField<String> = ObservableField("#ffffff")
    val charactersItemTextSize: ObservableField<Float> = ObservableField(0f)
    val charactersItemTextTypeface: ObservableField<String> = ObservableField("")
    ...
    init {
        DaggerMapperDependencyInjector.builder()
                ...
                .build()
                .inject(this)
        standardLeftAndRightMargin.set(convertDpToPixels(Constants.STANDARD_LEFT_AND_RIGHT_MARGIN))
        standardEditTextProductSmallMargin.set(convertDpToPixels(Constants.STANDARD_EDIT_TEXT_SMALL_MARGIN))
    ...
    private fun themeSwitcher(theme: MainActivity.UITheme) {
        uiDataClass?.let { uiDataClass ->
            ...
            charactersItemTextColor.set(colorMapper.map(theme, uiDataClass.charactersItemTextColor))
            charactersItemTextSize.set(textSizeMapper.map(theme, uiDataClass.charactersItemTextSize))
            charactersItemTextTypeface.set(typefaceMapper.map(theme, uiDataClass.charactersItemTextSize))
